...
public class ProgramChange extends ChannelEvent
{
    ...
    public String toString() {
        return String.format("%s %s", super.toString(), MidiProgram.fromProgramNumber(mValue1));
    }
    ...
    public enum MidiProgram
    {
        ...
        public static MidiProgram fromProgramNumber(int programNumber) {
            if (programNumber < 0 || programNumber >=    values().length)
                throw new IllegalArgumentException(
                        String.format("Invalid program number (%d)", programNumber));

            return values()[programNumber];
        }
